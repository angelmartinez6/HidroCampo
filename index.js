...
// Obtener TODOS los cultivos para la App
...
app.delete('/api/cultivos/:id', async (req, res) => {
  try {
    const { id } = req.params;
    await CultivoConfig.findByIdAndDelete(id);
    console.log("🗑️ Cultivo eliminado:", id);
...
app.post('/api/asistente', async (req, res) => {
  try {
    ...
    if (historialReciente.length > 0) {
      datosTexto = historialReciente.map(m => `pH: ${m.ph}, Temp: ${m.temperatura}°C, Caudal: ${m.caudal}L/min`).join(" | ");
    }

    const promptExperto = `
      Eres un ingeniero agrónomo experto. 
      CONTEXTO: Cultivo: ${configActiva.nombre} (${configActiva.etapa}), Terreno: ${configActiva.tamanoTerreno} Mz, Riego: ${configActiva.sistemaRiego}.
      DATOS: [ ${datosTexto} ]
      PREGUNTA: "${pregunta}"
...
app.listen(PORT, '0.0.0.0', () => console.log(`🚀 Servidor API en puerto ${PORT}`));
  ...
  try {
    ...
    if (configActiva) {
      ...
      if (matriz && matriz.parametros) {
        ...
        if (data.caudal < p.caudal.min) { consejosMatriz.push(`Profesional: ${p.caudal.bajo_prof}`); consejosMatriz.push(`Empírica: ${p.caudal.bajo_emp}`); } 
        else if (data.caudal > p.caudal.max) { consejosMatriz.push(`Profesional: ${p.caudal.alto_prof}`); consejosMatriz.push(`Empírica: ${p.caudal.alto_emp}`); }
      }
    }

    const nuevaMedicion = new Medicion({ ...data, recomendaciones: consejosMatriz });
