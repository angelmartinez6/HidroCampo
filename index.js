...
async function consultarGroq(prompt) {
  const response = await fetch(
    "https://api.groq.com/openai/v1/chat/completions",
    {
      ...
      body: JSON.stringify({
        model: "llama-3.3-70b-versatile", // Modelo ultra rápido y avanzado
...
client.on("message", async (topic, message) => {
  try {
    ...
    if (configActiva) {
      ...
      const matriz = await Recomendacion.findOne({
      ...
      if (matriz && matriz.parametros) {
        ...
        if (data.temperatura < p.temperatura.min) {
          ...
          consejosMatriz.push(
            `[TEMP_BAJO] Empírica: ${p.temperatura.bajo_emp}`,
          );
        } else if (data.temperatura > p.temperatura.max) {
          consejosMatriz.push(
            `[TEMP_ALTO] Profesional: ${p.temperatura.alto_prof}`,
          );
          consejosMatriz.push(
            `[TEMP_ALTO] Empírica: ${p.temperatura.alto_emp}`,
          );
        }
